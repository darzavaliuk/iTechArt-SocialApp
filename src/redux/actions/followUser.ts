...
export const followUserAction =
  ...
  async (dispatch: Dispatch<FollowUserAction>) => {
    try {
      ...
      await authRequest<any>('put', `${URI}/add-user`, { followUserId });
    } catch (error) {
      dispatch(
        followUserFail((error as AxiosError<{ message: string }>)?.response?.data?.message || 'Unexpected error')
      );
    }
  };
...
export const unfollowUserAction =
  ({ userId, users, followUserId }: FollowUnfollowParams) =>
  async (dispatch: Dispatch<UnfollowUserAction>) => {
    try {
      ...
      const updatedUsers = users.map((userObj) =>
        userObj._id === followUserId
          ? {
              ...userObj,
              followers: userObj.followers.filter((follower) => follower.userId !== userId),
            }
          : userObj
      );
      ...
      dispatch(
        unfollowUserFail((error as AxiosError<{ message: string }>)?.response?.data?.message || 'Unexpected error')
      );
    }
  };
