...
export const createPostAction =
  (title: string, image: string, user: User, replies: Array<{ title: string; image: string; user: User }>) =>
  async (dispatch: Dispatch<CreatePostsAction>) => {
    try {
      dispatch(createPostRequest());
      const response = await authRequest<PostData>('POST', `${URI}/create-post`, { title, image, user, replies });
