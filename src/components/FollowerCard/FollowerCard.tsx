  ...
  useEffect(() => {
    if (users) {
      if (followers) {
        ...
        const fullUsers = updatedUsers.filter((user: any) => followers.some((item: any) => item.userId === user._id));
        setData(fullUsers);
      }
      if (active === 1) {
        if (following) {
          const updatedUsers = [...users, user];
          const fullUsers = updatedUsers.filter((user: any) => following.some((item: any) => item.userId === user._id));
          setData(fullUsers);
        }
      }
    }
    ...
    <SafeAreaView>
      <View style={{ padding: 3, marginBottom: 2, position: 'relative' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={{ paddingLeft: 3, fontSize: 20, fontWeight: '600', color: '#000' }}>{item?.name}</Text>
        </View>
        <View
          style={{
            width: '80%',
        ...
        >
          <TouchableOpacity onPress={() => setActive(0)}>
            <Text style={{ fontSize: 18, paddingLeft: 3, color: '#000', opacity: active === 0 ? 1 : 0.6 }}>
              Followers
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActive(1)}>
            <Text style={{ fontSize: 18, paddingLeft: 3, color: '#000', opacity: active === 1 ? 1 : 0.6 }}>
              Following
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {active === 0 ? (
          <View
            style={{ width: '40%', position: 'absolute', height: 1, backgroundColor: 'black', left: -10, bottom: 0 }}
          />
        ) : active === 1 ? (
          <View
            style={{ width: '30%', position: 'absolute', height: 1, backgroundColor: 'black', right: '31%', bottom: 0 }}
          />
        ) : (
          <View
            style={{ width: '32%', position: 'absolute', height: 1, backgroundColor: 'black', right: 0, bottom: 0 }}
          />
        )}
      </View>
            ...
            return (
              ...
              >
                <View style={{ flexDirection: 'row' }}>
                  <Image source={{ uri: item?.avatar?.url }} style={{ width: 40, height: 40, borderRadius: 100 }} />
                  <View style={{ paddingLeft: 3 }}>
                    ...
                    <Text style={{ fontSize: 16, color: '#000000ba' }}>{item?.userName}</Text>
                  </View>
                </View>
                {user._id !== item._id && (
                  <TouchableOpacity
