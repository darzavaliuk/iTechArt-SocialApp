import { View, Text, SafeAreaView, TouchableOpacity, FlatList, Image, RefreshControl } from 'react-native';
...
const NotificationScreen = () => {
  ...
  const { notifications, isLoading } = useSelector((state: any) => state.notification);
            ...
            <View style={{ padding: 16, marginBottom: 190 }}>
              ...
              {active === 0 && notifications.length === 0 && (
                <View style={{ width: '100%', height: 80, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ fontSize: 16, color: 'black', marginTop: 5 }}>You have no activity yet!</Text>
                </View>
              )}

              {active === 1 && (
                <View style={{ width: '100%', height: 80, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ fontSize: 16, color: 'black', marginTop: 5 }}>You have no replies yet!</Text>
                </View>
              )}

              {active === 2 && (
                <View style={{ width: '100%', height: 80, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ fontSize: 16, color: 'black', marginTop: 5 }}>You have no mentions yet!</Text>
                </View>
              )}
                      ...
                      <TouchableOpacity onPress={() => handleNavigation(item)}>
                        <View style={{ flexDirection: 'row' }} key={item._id}>
                          <View style={{ position: 'relative' }}>
                            <Image
                              source={{
                                uri: users.find((user: any) => user._id === item.creator._id)?.avatar.url,
                              }}
