      ...
      {userProfile && posts && replies && (
        <View style={{ backgroundColor: 'white', flex: 1, marginHorizontal: 10 }}>
          ...
          >
            ...
            <TouchableOpacity>
              <View style={styles.statsContainer}>
                <View style={styles.statsBox}>
                  <Text style={[styles.text, { fontSize: 30 }]}>{userProfile.targets?.length || 0}</Text>
            ...
            {active === 0 && (
              <>{posts && posts?.map((item) => <PostCard postId={item._id} key={item._id} item={item} />)}</>
            )}
            ...
            {active === 1 && (
              <>
                {replies &&
                  ...
                  replies?.map((item) => <PostCard postId={item._id} key={item._id} item={item} replies={true} />)}
              </>
            )}
