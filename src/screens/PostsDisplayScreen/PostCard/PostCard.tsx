...
export const PostCard = ({ item, isReply, postId, replies }: Props) => {
  ...
  useEffect(() => {
    if (users) {
      ...
      const userData = updatedUsers.find((user: User) => user._id === item?.user?._id);
    ...
    >
      <View style={{ position: 'relative' }}>
        <View style={{ flexDirection: 'row', width: '100%' }}>
          <View style={styles.container}>
            <TouchableOpacity onPress={() => profileHandler(item.user)}>
              <Image source={{ uri: userInfo?.avatar?.url }} style={styles.avatar} />
            </TouchableOpacity>
            <View style={styles.infoContainer}>
              <TouchableOpacity style={styles.userInfoContainer} onPress={() => profileHandler(userInfo)}>
                <Text style={styles.userName}>{userInfo?.name}</Text>
                {userInfo?.role === 'Admin' && <Text style={styles.adminLabel}>Admin</Text>}
              </TouchableOpacity>
        ...
        <View style={styles.containerImage}>
          {item.image && <Image source={{ uri: item.image.url }} style={styles.image} resizeMode="contain" />}
        ...
        {replies && (
          <>
            {item?.replies?.map((reply: any) => (
              <PostDetailsCard navigation={navigation} key={reply._id} item={reply} isReply={true} postId={item._id} />
            ))}
          </>
        )}
        {openModal && (
                ...
                <View style={styles.overlay}>
                  <TouchableWithoutFeedback onPress={() => setOpenModal(true)}>
                    <View style={styles.modalContent}>
                      <TouchableOpacity style={styles.deleteButton} onPress={() => deletePostHandler(item._id)}>
