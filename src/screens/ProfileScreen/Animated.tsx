...
import { getAvatarTopMargin, getHeightForElements, getMarginForAndroid } from './Helpers';
...
export const getAnimatedImageSize = ({
  ...
  outputMinHeight,
  outputMaxHeight,
}: AnimatedHeightWithOutput): Animated.AnimatedInterpolation<number> =>
  animatedValue.interpolate({
    inputRange: [0, maxHeight - minHeight],
...
export const getAnimatedImagePosition = ({
  ...
  maxHeight,
  outputMaxHeight,
}: AnimatedHeightWithMaxOutput): Animated.AnimatedInterpolation<number> =>
  animatedValue.interpolate({
    inputRange: [0, maxHeight - minHeight],
    outputRange: [maxHeight - outputMaxHeight / 2, maxHeight + getAvatarTopMargin(minHeight)],
    extrapolate: 'clamp',
  });
...
export const getAnimatedUsernamePosition = ({
  ...
  avatarFontSize,
  headerFontSize,
}: AnimatedWithFontSizes): Animated.AnimatedInterpolation<number> => {
...
export const getAnimatedValues = (scrollY: Animated.Value, screenHeight: number) => {
  const { headerMaxHeight, headerMinHeight, imageMinHeight, imageMaxHeight, headerFontSize, avatarFontSize } =
