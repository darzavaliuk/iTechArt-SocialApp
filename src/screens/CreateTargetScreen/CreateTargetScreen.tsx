import { View, Text, TextInput, TouchableOpacity, FlatList, Alert, StyleSheet, Button } from 'react-native';
...
export const CreateTargetScreen = () => {
  ...
  function addText(text) {
    if (value !== '') {
      setList((prev) => {
        return [...prev, { text: text, way: selectedId, maxValue: maxValue }];
      });
      setValue('');
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        style={{ flex: 1 }}
        data={list}
        renderItem={({ item, index }) => <Card data={item} index={index} deleteItem={deleteItem} editItem={editItem} />}
